...
class CorrespondencesDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        print('Dataxs = ',xs.shape)
        print('Datays = ',ys.shape)
        print('DataR  = ',R.shape)
        print('Datat  = ',t.shape)
        side = []
        # use_fundamental=False    use_mutual=0, use_ransac=True, use_ratio=0
        ...
        #print('K1',K1, 'K2',K2, 'R',R, 't',t, 'xs',xs, 'ys',ys, 'T1',T1, 'T2',T2, 'virtPt',pts_virt, 'side',side)        
        print('data44__getitem__')
    ...
    def __len__(self):
        if self.data is None:
            ...
            self.data.close()
            self.data = None
            print('data66__len1__ = ',_len) # run here
        else:
            _len = len(self.data['xs'])
            print('data66__len2__= ',_len )
        print('data66__len__')
        return _len

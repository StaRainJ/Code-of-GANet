...
def test_process(mode, model, cur_global_step, data_loader, config):
    ...
    loader_iter = iter(data_loader) # 返回一个迭代器
    print("testLoader_Iter= ",loader_iter)
    ...
    with torch.no_grad(): 
        for test_data in loader_iter:
            ...
            res_logits, res_e_hat, res_mean = model(test_data)
            print("test_res_logits= ",res_logits)
            y_hat, e_hat, mean = res_logits[-1], res_e_hat[-1], res_mean[-1]  #### core print 
            print("y_hat= ",y_hat)
            print("e_hat= ",e_hat)
            mask = (y_hat > 0).type(torch.IntTensor)
            yhat_list += [mask.detach().cpu().numpy()]
            
            loss, geo_loss, cla_loss, l2_loss, prec, rec = match_loss.run(cur_global_step, test_data, y_hat, e_hat, mean)
    ...
    ofn = os.path.join(config.res_path, "time.txt")
    np.savetxt(ofn, np.array(avg_t))
    
    
    with open("label_atem.pkl", 'wb') as lf:
        pickle.dump(yhat_list, lf)
    with open("fundam_atem.pkl", 'wb') as ef:
        pickle.dump(ehat_list, ef)
    '''
...
def test(data_loader, model, config):  # model is class form from oan.py-->  model = Model(config)
    ...
    if not os.path.exists(save_file_best):
        ...
        exit(1)
    # Restore model
    checkpoint = torch.load(save_file_best) # 加载模型参数

...
class OANet(nn.Module):
    def __init__(self, config):
        ...
        self.weights_init = ATEM(config.net_channels, 4)
        self.weights_iter = [ATEM(config.net_channels, 6) for _ in range(config.iter_num)]
    ...
    def forward(self, data):
        ...
        print('OAN_batch_size = ',batch_size,'OAN_num_pts = ',num_pts)
        #data: b*1*n*c
        ...
        logits, e_hat, residual, mean = self.weights_init(data['xs']) ## model only need xs as input
        res_logits.append(logits), res_e_hat.append(e_hat), res_mean.append(mean)
        for i in range(self.iter_num):
            logits, e_hat, residual, mean = self.weights_iter[i](
                torch.cat([data['xs'], residual.detach(), torch.relu(torch.tanh(logits)).reshape(residual.shape).detach()], dim=3)
                )
            res_logits.append(logits), res_e_hat.append(e_hat), res_mean.append(mean)
        return res_logits, res_e_hat, res_mean 

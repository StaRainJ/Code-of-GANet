...
def test_sample(args):
    ...
    return [float(_err_q), float(_err_t), float(_num_inlier), _R_hat.reshape(1,-1), _t_hat.reshape(1,-1), t_diff]
...
def test_process(mode, model, cur_global_step, data_loader, config):
    ...
    with torch.no_grad(): 
        for test_data in loader_iter:
            ...
            t = end -start
            time_list.append(t)
            y_hat, e_hat, mean = res_logits[-1], res_e_hat[-1], res_mean[-1]
